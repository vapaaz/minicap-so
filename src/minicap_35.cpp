...
private:
  ...
  createVirtualDisplay() {
    ...
    mVirtualDisplay = android::SurfaceComposerClient::createVirtualDisplay(
      /* const std::string displayName */  std::string("minicap"),
      /* bool secure */                 false
    );
...
minicap_try_get_display_info(int32_t displayId, Minicap::DisplayInfo* info) {
  ...
  auto ids = android::SurfaceComposerClient::getPhysicalDisplayIds();
  for (auto id : ids) {
	dpy = android::SurfaceComposerClient::getPhysicalDisplayToken(id);
  }
  ...
  err = android::SurfaceComposerClient::getStaticDisplayInfo(static_cast<int64_t>(displayId), &dinfo);
